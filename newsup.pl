...
sub _launch_header_check{
  ...
  die_newsup ("Unable to print to socket" ) if (_print_args_to_socket($socket, "GROUP ", $newsgroup,$CRLF)!=0);
  ...
  if ($output =~ /^211\s/) {
	...
	die_newsup("Unable to print to socket") if (_print_args_to_socket($socket, "head <",$segment->{id},">",$CRLF));
...
sub _launch_upload{
  ...
  die_newsup ("Unable to login. Please check the credentials") if _authenticate($socket, $user, $password) >= 1;
  ...
  for my $segment (@$segments) {
    ...
    die_newsup ("Unable to print to socket") if (_print_args_to_socket($socket, "POST",$CRLF));
    my $output = _read_from_socket($socket);
    if ($output =~ /^340\s/) {
		
      die_newsup ("Unable to print to socket") if (_print_args_to_socket($socket,
...
sub _authenticate{
  ...
  die_newsup ("Unable to print to socket") if (_print_args_to_socket ($socket, "authinfo user ",$user,$CRLF) != 0);

  $output =  _read_from_socket $socket;
  die_newsup $output if $output !~ /381/;

  die_newsup ("Unable to print to socket") if (_print_args_to_socket ($socket, "authinfo pass ",$password,$CRLF) != 0);
...
sub _create_socket{
  ...
  $socket->autoflush(1);
  #my $timeout  = pack( 'l!l!', 5, 0 );#seconds, micro seconds (useconds)
  #$socket->setsockopt( SOL_SOCKET, SO_RCVTIMEO, $timeout ); #read/write timeout
  #$socket->setsockopt( SOL_SOCKET, SO_SNDTIMEO, $timeout ); #read/write timeout
  return $socket;
}
